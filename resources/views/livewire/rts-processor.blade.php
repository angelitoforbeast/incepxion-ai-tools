                    ...
                    @elseif (in_array($current->status, ['queued','scanning','processing']))
                        @if ($current->canceled_at)
                            <div class="flex items-center gap-2 text-sm text-amber-600">
                                <svg class="animate-spin w-4 h-4 text-amber-500" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z"></path></svg>
                                Canceling… stopping the process.
                            </div>
                        @else
