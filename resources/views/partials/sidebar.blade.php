@php
    $nav = [
        ...
        ['route' => 'profile',   'label' => 'Profile & API Key', 'icon' => 'M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z'],
    ];
    $user = auth()->user();
@endphp
        ...
        @if ($user->isAdmin())
            <a href="{{ url('/admin') }}"
               class="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-slate-300 hover:bg-slate-800 hover:text-white transition">
                ...
                Admin
                <span class="ml-auto text-[10px] bg-slate-700 rounded px-1.5 py-0.5">soon</span>
            </a>
        @endif
    </nav>
    ...
    <div class="border-t border-slate-800 p-3">
        <div class="flex items-center gap-3">
            ...
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" title="Log out" class="text-slate-400 hover:text-white p-1">
